...
class RiEmacs
   ...
   def initialize(paths)
     ...
     paths = RDoc::RI::Paths::raw_path(true,true,true,true,paths).find_all do |path|
       File.directory? path
     end
     

      @ri_reader = RDoc::RI::Reader.new(RDoc::RI::Cache.new(paths))
      @display = RDoc::RI::DefaultDisplay.new(options.formatter,options.width,options.use_stdout)
   end
